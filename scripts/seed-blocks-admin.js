// Script para sembrar bloques usando la API de administración de Strapi
...
async function createBlock(block) {
  try {
    const response = await axios.post(`${API_URL}/admin/content-manager/collection-types/api::block.block`, {
      block: block.block,
...
async function seedBlocks() {
  try {
    ...
    try {
      await axios.get(`${API_URL}/admin/content-manager/collection-types/api::block.block`, {
        headers: {
          'Authorization': `Bearer ${TOKEN}`
        }
      });
      console.log('Conexión exitosa a la API de administración');
    } catch (error) {
      console.error('Error de conexión a la API de administración:', error.response?.data?.error?.message || error.message);
      process.exit(1);
    }

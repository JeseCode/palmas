...
async function createUsers(strapi, roles) {
  try {
    ...
    for (const userData of usersToCreate) {
      ...
      if (!role) {
        strapi.log.info(`- No se encontró el rol ${userData.roleName}, se omitirá el usuario ${userData.username}`);
        continue;
      }
      ...
      strapi.log.info(`- Usuario ${userData.username} creado con rol ${userData.roleName}`);
    }
